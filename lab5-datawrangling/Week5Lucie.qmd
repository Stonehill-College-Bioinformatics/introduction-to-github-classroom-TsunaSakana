...
library(nycflights13)  
#flights
#flights$dep_delay[flights$dep_delay<= 12]
#flights %>% filter(dep_delay< -12)

flight2 <-flights %>% filter(dep_delay > 12)

origingroup <-flight2 %>% 
  count(origin)

flights %>% arrange(desc(dep_delay))
#group_by (origingroup)
origingroup %>% arrange(desc(n))
```
...
```{r}
#make new datafram, where origin==JFK and dest==LAX, then do calcs
library(nycflights13) 
JFK2LAX <- flights %>% filter(origin == "JFK" & dest == "LAX")
AvgairtimeJL <- JFK2LAX %>% 
  summarize(airtimeaverage = mean(air_time,na.rm = TRUE), numberofflights = nrow(JFK2LAX))
...
```{r}
library(dplyr)
#for each tzone, count and sort desc
#na.rm=false , filter by that somehow

Dest_tzone_freq <- airports %>% 
  count(tzone)

tzone_sorted <- Dest_tzone_freq %>%
  arrange(desc(n))


tzone_sorted

Missing_tzone <- airports %>% filter(is.na(tzone))
